...
const CourseArchivedPage = memo(function CourseArchivedPage() {
  const navigate = useNavigate();
  const archived = useMemo(() => MOCK_COURSES.filter((c) => c.lifecycle === 'archived'), []);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-section-gap)' }}>
      <div>
        ...
        <p style={{
          ...
          {archived.length} course{archived.length !== 1 ? 's' : ''} archived
        </p>
      </div>

      {archived.length === 0 ? (
        <CourseEmptyState type="no-archived" />
      ) : (
        <div style={{
          display: 'grid',
...
export default CourseArchivedPage;
