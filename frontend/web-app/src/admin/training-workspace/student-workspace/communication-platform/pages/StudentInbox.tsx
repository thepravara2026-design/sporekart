...
export default function StudentInbox() {
  const { inbox, getFilteredInbox, toggleStar, toggleRead } = useCommunication();
  ...
  return (
    <main style={{ padding: 'var(--space-3)', display: 'flex', flexDirection: 'column', gap: 'var(--space-section-gap)' }}>
      <div>
        ...
        <SharedFilters currentPage="communication/inbox" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 'var(--space-component-gap)' }}>
        ...
        <MetricCard label="Important" value={inbox.filter((i) => i.isImportant).length} icon="❗" color="#2563eb" />
      </div>
      ...
      <DashboardWidget title="Messages" subtitle={`${displayItems.length} messages`}>
