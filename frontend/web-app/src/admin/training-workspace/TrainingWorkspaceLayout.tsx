...
import { NAV_GROUPS, getTrainingActiveId, getTrainingLabel } from './data/navigation';

const TrainingWorkspaceLayout = memo(function TrainingWorkspaceLayout() {
  ...
  const segments = location.pathname.replace('/admin/training', '').split('/').filter(Boolean);
  const sectionId = segments[0] || 'dashboard';
  const sectionLabel = getTrainingLabel(sectionId);

  return (
    <div style={{
      display: 'flex', gap: 0, height: '100%', position: 'relative',
    }}>
      {/* Mobile overlay */}
      {navOpen && (
        <div
          ...
          className="training-nav-overlay"
        />
      )}

      {/* Navigation rail */}
      <nav
        className={`training-nav-rail ${navOpen ? 'training-nav-rail--open' : ''}`}
        style={{
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        {/* Mobile header */}
        <div className="training-mobile-header" style={{
      ...
      <style>{`
        @media (max-width: 767px) {
          ...
          .training-nav-rail--open {
            left: 0 !important;
          }
          .training-nav-overlay {
...
export default TrainingWorkspaceLayout;
