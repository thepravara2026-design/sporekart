...
export const BatchSummaryCards = memo(function BatchSummaryCards({ metrics }: { metrics: BatchMetric[] }) {
  return (
      ...
      {metrics.map((metric) => (
        <div key={metric.label} style={{ background: 'var(--color-surface)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--color-border)', padding: 16, display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            <span style={{ fontSize: 'var(--text-h1, 24px)', fontWeight: 700, color: 'var(--color-text-primary)' }}>{metric.value.toLocaleString()}</span>
            <span style={{ fontSize: 'var(--text-body, 14px)', color: VARIANT_COLORS[metric.variant] ?? 'var(--color-text-tertiary)' }}>
