...
export const BatchTimelineComponent = memo(function BatchTimelineComponent({ events }: { events: BatchTimelineEvent[] }) {
  ...
  if (events.length === 0) {
    return <div style={{ padding: 24, textAlign: 'center', color: 'var(--color-text-tertiary)' }}>No timeline events available.</div>;
  }
          ...
          <div key={event.id} style={{ display: 'flex', alignItems: 'stretch', gap: 12, minHeight: 56 }}>
            ...
            <div style={{ flex: 1, paddingBottom: idx < filtered.length - 1 ? 12 : 0 }}>
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 4, flexWrap: 'wrap' }}>
                {event.fromStatus && <span style={{ fontSize: 'var(--text-caption)', color: 'var(--color-text-secondary)', background: 'var(--color-surface-hover)', padding: '1px 8px', borderRadius: 'var(--radius-sm)' }}>{event.fromStatus.replace(/_/g, ' ')}</span>}
                {event.fromStatus && <span style={{ fontSize: 12, color: 'var(--color-text-tertiary)' }}>→</span>}
                <span style={{ fontSize: 'var(--text-caption)', color: 'var(--color-text-secondary)', background: 'var(--color-primary-alpha)', padding: '1px 8px', borderRadius: 'var(--radius-sm)' }}>{event.toStatus.replace(/_/g, ' ')}</span>
