...
interface MovementWorkspaceLayoutProps {
...
export const MovementWorkspaceLayout = memo(function MovementWorkspaceLayout({ activeSection, onSectionChange, searchQuery, onSearchChange, children }: MovementWorkspaceLayoutProps) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', background: 'var(--color-bg)' }}>
      <header style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', background: 'var(--color-surface)', borderBottom: '1px solid var(--color-border)', justifyContent: 'space-between', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <h2 style={{ margin: 0, fontSize: 'var(--text-h3, 18px)', fontWeight: 700 }}>Inventory Movements</h2>
        </div>
        <div style={{ position: 'relative', flex: 1, maxWidth: 360 }}>
          <input
            value={searchQuery}
            onChange={(e) => onSearchChange(e.target.value)}
            placeholder="Search by reference, product, warehouse..."
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <button style={{ padding: '6px 14px', border: 'none', borderRadius: 'var(--radius-sm)', background: 'var(--color-primary)', color: '#fff', cursor: 'pointer', fontSize: 'var(--text-caption)', fontWeight: 600 }}>+ New Movement</button>
        </div>
      </header>
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        <aside style={{ width: 220, flexShrink: 0, background: 'var(--color-surface)', borderRight: '1px solid var(--color-border)', overflowY: 'auto', padding: '12px 0', display: sidebarOpen ? 'block' : 'none' }}>
          <nav aria-label="Movement workspace sections" style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
