...
const ROLES: StockRole[] = ['viewer', 'inventory_operator', 'warehouse_operator', 'inventory_manager', 'administrator'];
...
export const StockPreviewApp = memo(function StockPreviewApp() {
  const [activeTab, setActiveTab] = useState('dashboard');
  const [viewport, setViewport] = useState<'desktop' | 'tablet' | 'mobile'>('desktop');
  const [theme, setTheme] = useState<'light' | 'dark'>('light');
  const [role, setRole] = useState<StockRole>('administrator');
  ...
  return (
    <StockWorkspaceProvider initialRole={role}>
      <div style={{ background: '#f8f9fa', minHeight: '100vh' }}>
        <header style={{ background: 'var(--color-surface, #fff)', borderBottom: '1px solid var(--color-border, #e0e0e0)', padding: '12px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontWeight: 700, fontSize: 16 }}>Stock Engine · Preview</span>
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <select value={role} onChange={(e) => setRole(e.target.value as StockRole)} style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid var(--color-border)', background: 'var(--color-surface)', color: 'var(--color-text-primary)' }}>
        ...
        <footer style={{ borderTop: '1px solid var(--color-border, #e0e0e0)', padding: '12px 24px', background: 'var(--color-surface, #fff)', textAlign: 'center', fontSize: 12, color: '#999' }}>
          SporeKart Stock Engine · Preview · Mock Mode · WCAG 2.2 AA · Responsive 320–1920px
        </footer>
      </div>
