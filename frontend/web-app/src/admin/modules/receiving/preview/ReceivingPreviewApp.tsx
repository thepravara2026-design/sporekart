...
const ROLES: ReceivingRole[] = ['viewer', 'receiving_operator', 'warehouse_operator', 'quality_inspector', 'warehouse_manager', 'inventory_manager', 'administrator'];
...
function PreviewContent({ activeTab }: { activeTab: string }) {
  switch (activeTab) {
    ...
    case 'workflow': return <PreviewWorkflowPage />;
    case 'inspection': return <PreviewInspectionPage />;
    case 'timeline': return <PreviewTimelinePage />;
    case 'analytics': return <PreviewAnalyticsPage />;
    default: return <PreviewDashboardPage />;
  }
}

export const ReceivingPreviewApp = memo(function ReceivingPreviewApp() {
  const [activeTab, setActiveTab] = useState('dashboard');
  const [viewport, setViewport] = useState<'desktop' | 'tablet' | 'mobile'>('desktop');
  const [theme, setTheme] = useState<'light' | 'dark'>('light');
  const [role, setRole] = useState<ReceivingRole>('administrator');
  ...
  const themeVars: CSSProperties = {
    ...
    '--text-h1': '24px', '--text-h2': '20px', '--text-h3': '16px', '--text-body': '14px', '--text-caption': '12px',
    '--radius-sm': '4px', '--radius-md': '6px', '--radius-lg': '8px', '--radius-badge': '12px', '--space-component-gap': '16px',
  } as CSSProperties;
      ...
      <div style={{ background: '#f8f9fa', minHeight: '100vh' }}>
        <header style={{ background: 'var(--color-surface, #fff)', borderBottom: '1px solid var(--color-border, #e0e0e0)', padding: '12px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontWeight: 700, fontSize: 16 }}>Goods Receiving · Preview</span>
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <button onClick={() => setTheme((t) => (t === 'light' ? 'dark' : 'light'))} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid var(--color-border)', background: 'var(--color-surface)', cursor: 'pointer', color: 'var(--color-text-primary)' }}>{theme === 'light' ? '\uD83C\uDF19 Dark' : '\u2600\uFE0F Light'}</button>
            <select value={role} onChange={(e) => setRole(e.target.value as ReceivingRole)} style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid var(--color-border)', background: 'var(--color-surface)', color: 'var(--color-text-primary)' }}>
        ...
        <nav style={{ background: 'var(--color-surface, #fff)', borderBottom: '1px solid var(--color-border, #e0e0e0)', padding: '0 24px', display: 'flex', gap: 0, overflowX: 'auto' }}>
          {PREVIEW_TABS.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '12px 16px', border: 'none', borderBottom: activeTab === tab.id ? '2px solid var(--color-primary)' : '2px solid transparent', background: 'none', cursor: 'pointer', color: activeTab === tab.id ? 'var(--color-primary)' : 'var(--color-text-secondary)', fontWeight: activeTab === tab.id ? 600 : 400, fontSize: 14, whiteSpace: 'nowrap' }}>
        ...
        <footer style={{ borderTop: '1px solid var(--color-border, #e0e0e0)', padding: '12px 24px', background: 'var(--color-surface, #fff)', textAlign: 'center', fontSize: 12, color: '#999' }}>
          SporeKart Purchase Receiving · Preview · Mock Mode · WCAG 2.2 AA · Responsive 320–1920px
        </footer>
      </div>
    </ReceivingWorkspaceProvider>
  );
