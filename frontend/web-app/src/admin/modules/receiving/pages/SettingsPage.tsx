...
const SETTINGS_GROUPS = [
  {
    ...
    items: [
      { label: 'Auto-approve accepted goods', key: 'autoApprove', type: 'toggle' as const },
      { label: 'Require inspection before acceptance', key: 'requireInspection', type: 'toggle' as const },
      { label: 'Enable batch assignment on receipt', key: 'autoBatch', type: 'toggle' as const },
      { label: 'Default receiving zone', key: 'defaultZone', type: 'select' as const, options: ['Receiving-A', 'Receiving-B', 'Receiving-C', 'Dock-1'] },
    ],
  },
  {
    title: 'Inspection Defaults',
    items: [
      { label: 'Visual inspection required', key: 'visualRequired', type: 'toggle' as const },
      { label: 'Quality inspection required', key: 'qualityRequired', type: 'toggle' as const },
      { label: 'Quantity verification required', key: 'qtyVerification', type: 'toggle' as const },
      { label: 'Documentation check required', key: 'docCheck', type: 'toggle' as const },
      { label: 'Max inspection duration (hours)', key: 'maxInspectHours', type: 'number' as const },
    ],
  },
  {
...
export const SettingsPage = memo(function SettingsPage() {
  const [settings, setSettings] = useState<Record<string, string | boolean | number>>({
    ...
    autoBatch: true,
    defaultZone: 'Receiving-A',
    visualRequired: true,
    qualityRequired: true,
    qtyVerification: true,
    docCheck: true,
    maxInspectHours: 48,
    rowsPerPage: '25',
