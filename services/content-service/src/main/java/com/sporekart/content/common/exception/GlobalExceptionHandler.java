...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetails> handleReviewNotFound(ReviewNotFoundException ex) {
        LOGGER.warn("Review not found: {}", ex.getMessage());
        return buildResponse(HttpStatus.NOT_FOUND, "review.notfound", ex.getMessage(), null);
    }

    @ExceptionHandler(ContentException.class)
    public ResponseEntity<ProblemDetails> handleContentException(ContentException ex) {
        LOGGER.error("Content exception", ex);
        return buildResponse(HttpStatus.BAD_REQUEST, "content.error", ex.getMessage(), null);
    }
