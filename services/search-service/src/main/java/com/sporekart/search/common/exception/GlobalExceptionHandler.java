...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetails> handleDocumentNotFound(DocumentNotFoundException ex) {
        LOGGER.warn("Document not found", ex);
        return buildResponse(HttpStatus.NOT_FOUND, "document.not.found", ex.getMessage(), null);
    }

    @ExceptionHandler(SearchException.class)
    public ResponseEntity<ProblemDetails> handleSearchException(SearchException ex) {
        LOGGER.error("Search exception", ex);
        return buildResponse(HttpStatus.BAD_REQUEST, "search.error", ex.getMessage(), null);
    }
