...
- Name: training-platform.spec.ts >> Phase 2 — Training Details >> Course detail has enroll CTA
- Location: tests\training-platform.spec.ts:141:7
...
Locator: locator('button:has-text("Enroll"),a:has-text("Enroll"),button:has-text("Join")').first()
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('button:has-text("Enroll"),a:has-text("Enroll"),button:has-text("Join")').first()
...
- link "Skip to content":
  - /url: "#main"
- heading "Course not found" [level=1]
- paragraph: The course you are looking for is not available.
